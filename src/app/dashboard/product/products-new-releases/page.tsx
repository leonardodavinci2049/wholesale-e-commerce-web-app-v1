...
interface ProductsNewReleasesPageProps {
...
export default async function ProductsNewReleasesPage({
  searchParams,
}: ProductsNewReleasesPageProps) {
  ...
  const [products, orderCart] = await Promise.all([
    productsPromise.catch((error) => {
      logger.error("Erro ao carregar produtos lançamentos:", error);
      return [];
    }),
    orderCartPromise,
  ]);
      ...
      <main className="flex flex-1 flex-col gap-4 p-4 pt-0 lg:p-6 lg:pt-0">
        <div className="mx-auto w-full max-w-350 flex flex-col gap-1 mb-2">
          ...
          <p className="text-muted-foreground text-sm md:text-base">
            Fique por dentro das últimas novidades e lançamentos do nosso
            catálogo.
          </p>
        </div>
        ...
        <div className="mx-auto grid w-full max-w-350 grid-cols-1 gap-4 xl:grid-cols-[minmax(0,1fr)_380px]">
          <div className="flex min-w-0 flex-col gap-4">
            <ProductViewSwitcher
              ...
              searchAriaLabel="Buscar lançamento"
              grid={
